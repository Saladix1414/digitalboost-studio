...
w = w.replace('from "./DigitalBoostCommandCenter"', 'from "./DigitalBoostCommandCenter.tsx"')
props = 'onOpenAI={() => setShowAI(true)}'
full = 'onOpenAI={() => setShowAI(true)} onOpenHealth={() => setShowHealth(true)} onOpenIntegrations={() => setShowIntegrations(true)} onOpenAutomations={() => setShowAutomations(true)} onOpenConsole={() => setShowConsole(true)} onOpenNotes={() => setShowNotes(true)} onOpenShortcuts={() => setShowShortcuts(true)}'
if "onOpenAI={() => setShowAI(true)}" in w:
    # leave existing extra props; append missing ones one by one
    for name, snippet in [
        ("onOpenHealth", " onOpenHealth={() => setShowHealth(true)}"),
        ("onOpenIntegrations", " onOpenIntegrations={() => setShowIntegrations(true)}"),
        ("onOpenAutomations", " onOpenAutomations={() => setShowAutomations(true)}"),
        ("onOpenConsole", " onOpenConsole={() => setShowConsole(true)}"),
        ("onOpenNotes", " onOpenNotes={() => setShowNotes(true)}"),
        ("onOpenShortcuts", " onOpenShortcuts={() => setShowShortcuts(true)}"),
    ]:
        if name not in w:
            w = w.replace("onOpenAI={() => setShowAI(true)}", "onOpenAI={() => setShowAI(true)}" + snippet, 1)
ws.write_text(w, encoding="utf-8")
print("ok workspace props")
print("LISTO CMD ALL")
print("Ctrl+K lista completa")
