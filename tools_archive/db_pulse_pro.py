...
export function analyze(input: PulseInput): PulseDecision {
  ...
  if (builder) {
    ...
    if (q.indexOf("theme") !== -1 || q.indexOf("noir") !== -1 || q.indexOf("color") !== -1)
      return { title: "PULSE Design · Theme", body: "Aura = calma. Noir = precisio. El chip Theme aplica tokens al telefono.", action: "website-builder", actionLabel: "Seguir en el canvas", confirm: false };
...
if "const CHIPS" in ot:
    ot = ot.replace(
        'const CHIPS = builder ? ["hola", "hero"] : ["hola", "ventas", "pedidos", "health", "buscar"];',
        'const CHIPS = builder ? ["hola", "hero", "theme", "conversion"] : ["hola", "ventas", "pedidos", "stock", "health", "buscar"];',
        1,
    )
op.write_text(ot, encoding="utf-8")
print("ok chips")
print("LISTO PULSE PRO")
print("6s max. Si Llama no llega, reglas. El boton NO se queda pensando.")
