...
has_log = Path("src/DigitalBoostConsoleData.ts").is_file()
log_imp = 'import { pushLog } from "./DigitalBoostConsoleData";\n' if has_log else ""
log_call = 'try { pushLog({ actor: "PULSE", action: out.title, resource: section, status: "completed", result: String(out.body).slice(0, 120) }); } catch {}' if has_log else ""
...
export default function DigitalBoostOperator(props: {
  ...
  async function think(word: string) {
    ...
    try {
      ...
      setEngine("rules");
      setOut({ title: "PULSE", body: "No pude hablar con el modelo. Seguí con reglas. " + String(e), action: "dashboard", actionLabel: "Cerrar", confirm: false });
    }
    setBusy(false);
  }
  function exec() {
