...
function analyze(q: string, section: string) {
  ...
  if (s.indexOf("hola") !== -1 || s.indexOf("quien") !== -1 || s.indexOf("sos") !== -1) {
    return builder
      ...
      : { title: "PULSE", body: "Estoy en Commerce OS. Opero ventas, stock, pedidos y campanas de este comercio.", action: "dashboard", actionLabel: "Seguir en Overview", confirm: false };
  }
  if (builder) {
    ...
    if (s.indexOf("theme") !== -1 || s.indexOf("color") !== -1 || s.indexOf("noir") !== -1)
      return { title: "PULSE Design · Theme", body: "Aura vende calma. Noir vende precisio. Cambia el preset desde Theme.", action: "website-builder", actionLabel: "Abrir Theme", confirm: false };
...
if "section={section}" not in w:
    w = w.replace(
        "<DigitalBoostOperator onClose={() => setShowAI(false)}",
        "<DigitalBoostOperator section={section} onClose={() => setShowAI(false)}",
        1,
    )
    w = w.replace(
        "<DigitalBoostOperator onClose={() => setShowAI(false)} onNavigate",
        "<DigitalBoostOperator section={section} onClose={() => setShowAI(false)} onNavigate",
        1,
    )
    if "section={section}" not in w:
        w = w.replace(
            "<DigitalBoostOperator ",
            "<DigitalBoostOperator section={section} ",
            1,
        )
ws.write_text(w, encoding="utf-8")
print("ok workspace")
print("LISTO PULSE CTX")
print("OS -> operador  |  Store Builder -> PULSE Design")
