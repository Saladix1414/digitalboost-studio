...
if not op.is_file():
    raise SystemExit("Falta DigitalBoostOperator.tsx — corre db_ai_open.py antes")
...
""", encoding="utf-8")
print("ok pulse operator")

cc = src / "DigitalBoostCommandCenter.tsx"
if cc.is_file():
    c = cc.read_text(encoding="utf-8")
    c = c.replace("Open AI Operator", "Open PULSE")
    c = c.replace("label: \"Open PULSE\", k: \"ia ai\"", "label: \"Open PULSE\", k: \"ia ai pulse operador\"")
...
if ws.is_file():
    ...
    w = w.replace("AI Operator", "PULSE")
    w = w.replace("AI Store Operator", "PULSE")
    ws.write_text(w, encoding="utf-8")
    print("ok workspace labels")

des = src / "DigitalBoostStoreDesigner.tsx"
if des.is_file():
    d = des.read_text(encoding="utf-8")
    d = d.replace("AI Store Designer", "PULSE Design")
    d = d.replace("Create · Modify · Suggest", "Diseno con PULSE")
    des.write_text(d, encoding="utf-8")
    print("ok designer brand")

print("LISTO PULSE")
print("Cmd / boton violeta -> PULSE")
