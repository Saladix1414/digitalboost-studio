...
export default function StoreBuilderWorkspace({
  ...
  const addProduct = () => {
    setProducts((current) => [
      ...current,
      {
        id: Date.now(),
        name: "Nuevo producto",
        sku: `DB-${Date.now().toString().slice(-5)}`,
        price: 49,
        stock: 25,
        status: "Borrador",
  ...
  const renderProducts = () => (
    <div className="space-y-6">
      <PageTitle
        title="Productos"
        subtitle="Administra todo tu catálogo."
        action={
          <button
            onClick={() => setShowAddProduct(true)}
            className="flex items-center gap-2 rounded-lg bg-gradient-to-r from-violet-600 to-cyan-400 px-4 py-2.5 text-sm font-bold"
          >
            <Plus size={16} />
            Añadir producto
          </button>
      ...
      <Panel>
        <div className="mb-5 flex flex-col gap-3 md:flex-row">
          ...
          <button className="rounded-lg border border-white/10 px-4 text-xs text-slate-300">
            Todos los productos
          </button>
        </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#02050d] text-white">
      ...
      {showAddProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#070d18]">
            ...
            <div className="space-y-4 p-5">
              ...
              <button
                onClick={addProduct}
                className="w-full rounded-lg bg-gradient-to-r from-violet-600 via-fuchsia-500 to-cyan-400 py-3 text-sm font-bold"
              >
                Crear producto
              </button>
            </div>
