...
import DigitalBoostCommandCenter from "./DigitalBoostCommandCenter.tsx";
...
import "./digitalboost-dashboard.css";
type StoreSection =
  | "dashboard"
  | "products"
  | "inventory"
...
const filteredProducts = useMemo(() => {
              ...
              <div className="rounded-xl border border-violet-400/20 bg-violet-500/20 p-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-[#475569]">
                      Analizá este pedido con PULSE.
                    </p>
                  </div>
                </div>

                <p className="mt-4 text-xs leading-6 text-[#475569]">
                  El PULSE podrá analizar el valor, cliente, productos,
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-[#475569]">
            Campañas creadas por el PULSE o manualmente.
          </p>
        </div>
      ...
      {campaigns.length === 0 ? (
        <Panel title="Campañas activas">
          <div className="p-6 text-center">
            ...
            <p className="mt-1 text-xs text-[#475569]">
              Creá una campaña desde el PULSE.
            </p>
          </div>
        </Panel>
      ) : (
  ...
  return (
    <div data-store-builder-vibrant="true" data-store-builder={section === "website-builder" ? "true" : "false"} data-os-section={section} className={`flex h-screen overflow-hidden ${section === "website-builder" ? "bg-[#070d18] text-[#F7FAFF]" : "bg-[#0A1020] text-white"}`}>
      {mobileMenu && (
        <div
      ...
      {showAI && (<DigitalBoostOperator onOpenSearch={() => setShowSearch(true)} onOpenHealth={() => setShowHealth(true)} onOpenAutomations={() => setShowAutomations(true)} onOpenConsole={() => setShowConsole(true)} section={section} onClose={() => { setShowAI(false); }} onNavigate={(id) => { setShowAI(false); setSection(id); }} />)}
      {false && (
          ...
          <div className="w-full max-w-lg rounded-2xl border border-violet-400/20 bg-[#0B1B30] shadow-2xl">
              ...
              <div className="border-t border-[#1D4260] p-5">
                <div className="rounded-xl border border-emerald-400/20 bg-emerald-500/5 p-4">
                  <div className="mb-4">
                    ...
                    <p className="mt-1 text-xs leading-5 text-[#475569]">
                      Configurá la campaña preparada por el PULSE.
                    </p>
                  </div>
            ...
            <div className="border-t border-[#1D4260] p-5">
              <div className="rounded-lg border border-[#00B7FF]/30 bg-[#0B1B30] p-3 text-xs leading-5 text-[#475569]">
                El PULSE analiza ventas, inventario, clientes y
          ...
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-[#00B7FF]/30 bg-[#0B1B30] shadow-2xl">
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="rounded-lg border border-violet-400/10 bg-violet-500/20 p-3 text-xs leading-5 text-[#475569]">
                ...
                imágenes, inventario, pedidos, analytics y PULSE.
              </div>
