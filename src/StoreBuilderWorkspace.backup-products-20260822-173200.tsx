...
export default function StoreBuilderWorkspace({
  ...
  const renderProducts = () => (
    <div className="space-y-6">
      <PageTitle
        title="Productos"
        subtitle={`${products.length} productos en tu catálogo.`}
        action={
          <button
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Metric
          title="Activos"
          value={String(products.filter((p) => p.status === "Activo").length)}
        />
        <Metric
          title="Stock bajo"
          value={String(products.filter((p) => p.stock > 0 && p.stock < 10).length)}
        />
        <Metric
          title="Agotados"
          ...
          <div className="flex gap-2 overflow-x-auto">
            {[
              ...
              <button
                ...
                onClick={() =>
                  setProductFilter(
                    id as "all" | "active" | "draft" | "low"
                  )
                }
                className={`whitespace-nowrap rounded-lg border px-4 py-2 text-xs transition ${
                  productFilter === id
                  ...
                  <td className="text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="rounded-md border border-white/10 px-3 py-1.5 text-xs text-slate-300 hover:border-cyan-400/30 hover:text-cyan-300"
                      >
                        Editar
                      </button>
                      ...
                      <button
                        ...
                        className="rounded-md border border-red-400/10 px-3 py-1.5 text-xs text-red-400 hover:border-red-400/30"
                      >
                        Eliminar
                      </button>
                    </div>
          ...
          <div className="w-full max-w-lg rounded-2xl border border-violet-400/20 bg-[#070d18] shadow-2xl">
            ...
            <div className="space-y-3 p-5">
              {[
                ...
                <button
                  key={prompt}
                  onClick={() => alert(`🤖 Analizando: ${prompt}`)}
