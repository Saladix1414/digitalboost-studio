...
function closeMissionOutcome(
  ...
  input: { status: PulseMissionTerminalOutcome; lastError?: string; verificationStatus?: string; verified?: boolean },
): PulseMission {
  if (mission.outcome) return mission;
  const plan = closePulsePlan(mission.plan, input.status);
  const id = outcomeId(mission.id, input.status);
  const recordedAt = new Date().toISOString();
  const outcome: PulseMissionOutcome = {
    id, status: input.status, missionId: mission.id, planId: mission.planId,
    requestId: mission.requestId, planStatus: plan.status,
    terminalStepIndex: mission.stepIndex, recordedAt,
    lastError: input.lastError, verificationStatus: input.verificationStatus,
    verified: input.verified,
  };
  try {
    rememberMissionOutcome({
      scope: mission.store, missionId: mission.id, planId: mission.planId,
      requestId: mission.requestId, outcome: input.status,
      planStatus: plan.status, terminalStepIndex: mission.stepIndex,
      lastError: input.lastError, verificationStatus: input.verificationStatus,
      verified: input.verified,
    });
  } catch {}
  try {
    if (!hasPulseMissionOutcomeAudit(mission.id, id)) {
      pushAudit({
        timestamp: recordedAt, tenant_id: "digitalboost", store_id: mission.store,
        actor_type: input.status === "CANCELLED" ? "merchant" : "system",
        request_id: mission.requestId, intent: "mission-outcome", agent: "pulse",
        risk_level: mission.plan.goal.riskFloor, tool: "mission",
        approval_required: false, status: input.status,
        result_summary: "PULSE_MISSION_OUTCOME:" + input.status,
        mission_id: mission.id, plan_id: mission.planId,
        step_index: mission.stepIndex, mission_outcome: input.status,
        plan_status: plan.status, outcome_id: id,
        verification_status: input.verificationStatus, verified: input.verified,
      });
    }
  } catch {}
  return save(Object.assign({}, mission, { plan: plan, outcome: outcome }));
}
export function startPulseMission(input: { store: string; plan: PulsePlan; requestId?: string }): PulseMission {
...
export function advancePulseMission(
  id: string,
  input: { approved?: boolean; ok?: boolean; error?: string; terminal?: boolean; verificationStatus?: string; verified?: boolean } = {},
): PulseMission | null {
  const m = getPulseMission(id);
  if (!m) return null;
  if (m.state === "CANCELLED" || m.state === "COMPLETED" || m.state === "FAILED") return m;
  if (m.state === "PAUSED") return null;

  const step = m.plan.steps[m.stepIndex];
  if (!step) return closeMissionOutcome(Object.assign({}, m, { state: "COMPLETED" }), {
    status: "COMPLETED", verificationStatus: input.verificationStatus, verified: input.verified,
  });

  if (m.state === "AWAITING_APPROVAL" && !input.approved) return m;

  if (input.ok === false) {
    const error = input.error || (input.terminal ? "step-failed-terminal" : "step-failed");

    if (input.terminal || m.retries + 1 > m.maxRetries) {
      return closeMissionOutcome(Object.assign({}, m, {
        state: "FAILED", lastError: error,
      }), {
        status: "FAILED", lastError: error,
        verificationStatus: input.verificationStatus, verified: input.verified,
      });
    }
  ...
  if (nextIndex >= m.plan.steps.length) {
    return closeMissionOutcome(Object.assign({}, m, {
      stepIndex: nextIndex, state: "COMPLETED", lastError: undefined,
    }), {
      status: "COMPLETED",
  ...
  return save(Object.assign({}, m, {
    stepIndex: nextIndex,
    state: nxt && nxt.approvalLikely ? "AWAITING_APPROVAL" : "RUNNING",
  }));
}
