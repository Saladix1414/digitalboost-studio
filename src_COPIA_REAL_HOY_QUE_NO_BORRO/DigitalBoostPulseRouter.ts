...
export function routeTools(input: PulseInput, q: string, finish: (i: PulseInput, p: any) => PulseDecision): PulseDecision | null {
  ...
  if (hit(q, ['inspecc', 'auditar', 'score', 'puntaje'])) return finish(input, { title: 'PULSE IA', body: toolScoreLine(f) + '. ' + f.store + ' · ' + f.page + ' · ' + f.blocks + ' bloques. Hero: «' + (f.heroTitle || '-') + '». ' + (f.notes.join('. ') || 'Sin notas.'), action: input.section === 'website-builder' ? 'website-builder' : 'dashboard', label: 'Seguir' });
  if (hit(q, ['mapa', 'bloques', 'estructura', 'outline'])) return finish(input, { title: 'PULSE IA', body: toolMap(f), action: 'website-builder', label: 'Abrir canvas' });
  ...
  if (hit(q, ['theme', 'noir', 'nimbus', 'nimbus', 'preset'])) return finish(input, toolTheme(f));
  if (hit(q, ['publicar', 'publish', 'lanzar'])) { const ok = f.score >= 70 && !f.genericHero; return finish(input, { title: 'PULSE IA', body: toolScoreLine(f) + '. ' + (ok ? 'Fold listo para Preview. Publicar no dispara campana.' : 'Todavia no: ' + (f.notes.join(', ') || 'sube el score') + '.'), action: 'website-builder', label: 'Seguir' }); }
