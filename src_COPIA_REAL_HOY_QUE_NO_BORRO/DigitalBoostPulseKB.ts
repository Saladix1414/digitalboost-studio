...
export function decide(input: PulseInput): PulseDecision {
  ...
  add(["hola", "quien", "sos", "pulse"], "PULSE IA", st(input) + ". Orquestador del OS. ¿Briefing del dia, un pedido, o el canvas?", "dashboard", "Seguir aca");
  ...
  if (!packs.length) {
    ...
    else pick = { title: "PULSE IA", body: "No enganché un frente claro. ¿Lo vemos por ventas, por el 1047, o por el hero del canvas?", action: input.section || "dashboard", label: "Seguir acá", score: 0 };
  } else {
    pick = packs[0];
  }
