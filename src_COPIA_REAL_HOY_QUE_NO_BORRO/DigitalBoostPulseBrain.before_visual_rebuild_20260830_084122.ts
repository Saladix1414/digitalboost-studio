...
export function analyze(input: PulseInput): PulseDecision {
  ...
  if (!q || q === "hola" || q.indexOf("quien") !== -1 || q.indexOf("sos") !== -1) {
    ...
    return { title: "PULSE IA", body: stamp + ". Operador de este OS. Pedime ventas, pedidos, stock, health, buscar o una campana. Si Ollama esta vivo, pienso con Llama; si no, con reglas.", action: "dashboard", actionLabel: "Seguir en Overview", confirm: false };
  }
  if (q.indexOf("gracias") !== -1)
    return { title: "PULSE IA", body: "De nada. Cuando quieras, health o el canvas.", action: section, actionLabel: "Seguir", confirm: false };
  ...
  if (q.indexOf("live") !== -1)
    return { title: "PULSE IA", body: stamp + ". Live/Attention esta en el header. Un toque lo cambia.", action: "dashboard", actionLabel: "Overview", confirm: false };

  return { title: "PULSE IA", body: stamp + ". Frente " + section + ". Proba: ventas, pedidos, stock, health, buscar, hero, campana.", action: section, actionLabel: "Seguir aca", confirm: false };
}
...
export async function analyzeSmart(input: PulseInput): Promise<{ decision: PulseDecision; engine: "ollama" | "rules" }> {
  ...
  try {
    ...
    return {
      engine: "ollama",
      decision: {
        title: String(parsed.title || "PULSE IA").slice(0, 80),
