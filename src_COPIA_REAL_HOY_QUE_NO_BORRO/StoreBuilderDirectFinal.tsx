                      ...
                      {index === 2 && (
                        <div className="dbf-products">
                          <div className="dbf-products-heading">
                            <div>
                              <span>COLECCIÓN</span>
                              <div style={{position:'relative'}}><strong>Destacados</strong>
      <button
        className="trash-right-final"
        onClick={(e:any)=>{
          e.stopPropagation();
          const blockEl = e.currentTarget.closest('[data-block-id]') || e.currentTarget.parentElement?.parentElement;
          const blockId = blockEl?.getAttribute?.('data-block-id');
          if(!confirm('¿Borrar este bloque?')) return;
          Object.keys(localStorage).filter(k=>k.includes('db-store-canvas')).forEach(k=>{
            try{
              let bs=JSON.parse(localStorage.getItem(k)||"[]");
              if(blockId) bs=bs.filter((b:any)=> String(b.id)!==blockId);
              else bs=bs.slice(0,-1);
              localStorage.setItem(k, JSON.stringify(bs));
            }catch{}
          });
          location.reload();
        }}
        style={{position:'absolute', top:'-8px', right:'-8px', zIndex:50, background:'#ff3b3b', color:'white', border:'2px solid white', borderRadius:'50%', width:'34px', height:'34px', cursor:'pointer'}}
      >🗑️</button></div>
                            </div>
