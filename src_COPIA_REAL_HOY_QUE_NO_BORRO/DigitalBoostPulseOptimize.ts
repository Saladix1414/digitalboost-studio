...
export function pulseOptimizeOnce(input?: PulseInput) {
  ...
  if(!parts.length) return {title:"PULSE IA",body:"Health "+seo.health+"/100. Limpio.",action:"seo",label:"Abrir SEO"};
  return {title:"PULSE IA",body:"Hecho: "+parts.join(" + ")+". Health "+seo.health+" → revisa SEO y canvas.",action:"seo",label:"Ver resultado"};
}
