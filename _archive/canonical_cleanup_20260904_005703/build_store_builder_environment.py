...
SHELL = SRC / "StoreBuilderEnvironment.tsx"

print("=" * 70)
print("DIGITALBOOST — STORE BUILDER ENVIRONMENT")
print("CONEXIÓN SEGURA DEL APPLICATION SHELL")
print("=" * 70)
...
backup = SRC / f"StoreBuilderWorkspace.tsx.before_environment_{stamp}.bak"
...
shell_code = r'''import React, { useEffect, useState } from "react";
import {
  Activity,
  Bot,
  CheckCircle2,
...
export default function StoreBuilderEnvironment({
  ...
  if (booting) {
    return (
      <div className="fixed inset-0 z-[9999] flex items-center justify-center overflow-hidden bg-[#02050b] text-white">
        ...
        <div className="relative w-full max-w-xl px-6">
          ...
          <div className="mt-10 rounded-2xl border border-white/[0.07] bg-[#050a12]/90 p-4 shadow-2xl backdrop-blur-xl">
  ...
  return (
    <div className="min-h-screen bg-[#01040a] text-white">
      ...
      <div className="relative flex min-h-screen flex-col">
        {/* APPLICATION BAR */}
        <header className="sticky top-0 z-50 h-14 border-b border-white/[0.07] bg-[#030711]/95 backdrop-blur-2xl">
        ...
        <div className="relative flex flex-1">
          {leftPanel && (
            <aside className="hidden w-14 shrink-0 border-r border-white/[0.06] bg-[#02060d] lg:flex lg:flex-col lg:items-center lg:py-3">
      ...
      {commandOpen && (
        ...
        >
          <div
            className="w-full max-w-xl overflow-hidden rounded-2xl border border-white/10 bg-[#050a12] shadow-2xl"
...
print("✓ StoreBuilderEnvironment.tsx creado")
...
if 'import StoreBuilderEnvironment from "./StoreBuilderEnvironment";' not in text:
    # Add after the final import statement.
    ...
    print("✓ Import ya existente")

# ------------------------------------------------------------
# LOCATE RENDER SECTION FUNCTION
# ------------------------------------------------------------

render_marker = "const renderSection = () => {"

render_start = text.find(render_marker)

if render_start == -1:
    raise SystemExit(
        "❌ No encontré renderSection. "
        "El archivo original queda intacto."
    )

# ------------------------------------------------------------
# IMPORTANT:
# We do NOT use the first `return (` after renderSection.
#
# renderSection itself contains many nested JSX returns.
#
# Instead, locate the end of renderSection by searching for:
#
#   };
#
# followed by:
...
#   return (
#
# at the component indentation level.
# ------------------------------------------------------------

component_return_pattern = re.compile(
    r'\n\s{2}\};\s*\n\s{2}return\s*\(',
    re.MULTILINE,
)

candidate = component_return_pattern.search(
    text,
    render_start + len(render_marker)
)

if not candidate:
    raise SystemExit(
        "❌ No encontré el return principal del componente. "
        "No se modificó StoreBuilderWorkspace.tsx."
    )

main_return = candidate.end() - len("return (")

print("✓ Return principal localizado")

# ------------------------------------------------------------
# FIND ROOT JSX ELEMENT
# ------------------------------------------------------------

after_return = main_return + len("return (")

root_match = re.search(
    r'\n\s*<([A-Za-z][A-Za-z0-9.]*)',
    text[after_return:after_return + 5000],
)

if not root_match:
    raise SystemExit(
        "❌ No pude localizar el elemento raíz JSX."
    )

root_tag = root_match.group(1)
root_start = after_return + root_match.start(1) - 1

print(f"✓ Root JSX detectado: <{root_tag}>")

# ------------------------------------------------------------
# MATCH JSX ROOT
# ------------------------------------------------------------

tail = text[root_start:]

token_pattern = re.compile(
    rf'<{re.escape(root_tag)}(?:\s[^>]*)?>|</{re.escape(root_tag)}>',
    re.MULTILINE,
)
...
if root_end is None:
    raise SystemExit(
        f"❌ No pude encontrar el cierre real de <{root_tag}>. "
        "No se modificó StoreBuilderWorkspace.tsx."
    )

# ------------------------------------------------------------
# PROTECTION AGAINST DOUBLE WRAP
# ------------------------------------------------------------

if "<StoreBuilderEnvironment>" in text:
    print("⚠️ El workspace ya parece estar envuelto.")
    print("No se realizará una segunda envoltura.")
else:
    opening = "<StoreBuilderEnvironment>\n"
    closing = "\n</StoreBuilderEnvironment>"

    text = (
        text[:root_start]
        + opening
        + text[root_start:root_end]
        + closing
        + text[root_end:]
    )
    ...
    print("✓ StoreBuilderWorkspace conectado al Environment")
...
print("✓ Store Builder Environment conectado")
print("✓ Boot experience preparado")
print("✓ Application shell preparado")
print("✓ Command Center preparado")
print("✓ Preview toggle preparado")
print("✓ Rail lateral preparado")
print("✓ Status bar preparada")
print()
