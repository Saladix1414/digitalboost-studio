...
SRC = ROOT / 'src'
TARGET = SRC / 'CommerceOSOverview.tsx'
BACKUP = TARGET.with_name(f'CommerceOSOverview.tsx.before_jewel_{datetime.datetime.now():%Y%m%d_%H%M%S}.bak')
...
export default function CommerceOSOverview({ onNavigate }: CommerceOSOverviewProps) {
  ...
  if (!booted) {
    return (
      <div className="relative min-h-[720px] overflow-hidden rounded-[28px] border border-white/10 bg-[#02050b] text-white shadow-2xl shadow-cyan-950/20">
...
print('=' * 70)
print('DIGITALBOOST — COMMERCE OS JEWEL UI')
print('NUEVA CAPA VISUAL · WEB DENTRO DE LA WEB · BOOT · AI · LIVE')
print('=' * 70)
...
print('✓ CommerceOSOverview.tsx reemplazado por la nueva interfaz Jewel')

print('\n' + '=' * 70)
print('BUILD DE VERIFICACIÓN')
print('=' * 70)
try:
    result = subprocess.run(['npm','run','build'], cwd=ROOT, text=True, capture_output=True, timeout=180)
except subprocess.TimeoutExpired:
    print('❌ npm run build excedió el tiempo límite.')
    if BACKUP.exists(): shutil.copy2(BACKUP, TARGET); print('✓ Restaurado backup')
    sys.exit(1)
...
print('\n' + '=' * 70)
print('COMMERCE OS JEWEL INSTALADO')
print('=' * 70)
print('✓ Boot / pantalla de inicialización')
print('✓ Web dentro de la web')
print('✓ App chrome interno')
print('✓ Sidebar profesional')
print('✓ Store Live + preview responsive')
print('✓ Business Pulse')
print('✓ AI Store Operator')
print('✓ Live Feed')
print('✓ Command search')
print('✓ Mobile navigation')
print('✓ Website Builder conectado por navegación')
