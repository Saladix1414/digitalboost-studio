...
export default function CommerceOSOverview({
  ...
  const metrics = [
    { label: "Ventas", value: money(Math.round((sales || 474) * osMul)), delta: "+18,4%", tone: "#34d399", pct: 0.74, go: "analytics" },
    { label: "Pedidos", value: String(Math.max(1, Math.round((orders.length || 4) * osMul))), delta: "+12,1%", tone: "#22d3ee", pct: 0.42, go: "orders" },
    { label: "Clientes", value: String(Math.max(1, Math.round((customers.length || 4) * osMul))), delta: "+9,7%", tone: "#a78bfa", pct: 0.51, go: "customers" },
  ...
  return (
    <div data-commerce-os="true" data-commerce-os-core="true" data-commerce-os-vibrant="true" className="relative min-h-[760px] overflow-hidden rounded-[28px] border border-white/10 bg-[#0A1020] text-[#F7FAFF]">
      ...
      <div className="relative mx-auto max-w-6xl px-4 py-6 sm:px-6">
        <section className="mb-6 grid items-end gap-6 lg:grid-cols-[1.3fr_0.7fr]">
          <div>
            ...
            <h1 className="mt-3 text-4xl font-semibold leading-[0.95] tracking-tight text-white sm:text-5xl">
              Tu comercio<span className="mt-1 block text-cyan-300">en movimiento.</span>
            </h1>
            <div className="mt-4 h-1 w-24 bg-gradient-to-r from-cyan-400 via-violet-500 to-pink-500" />
            <p className="mt-4 max-w-md text-sm leading-6 text-slate-400">Operá, diseñá y publicá desde un OS navy. El Store Builder es el estudio; este plano es el cerebro.</p>
            <div className="mt-6 flex flex-wrap gap-2">
              <button type="button" onClick={() => onNavigate?.("website-builder")} className="inline-flex h-11 items-center rounded-lg bg-cyan-400 px-5 text-sm font-semibold text-[#070d18]">Abrir Store Builder</button>
              <button type="button" onClick={() => onNavigate?.("orders")} className="inline-flex h-11 items-center rounded-lg border border-white/10 px-5 text-sm">Ver pedidos</button>
            </div>
          </div>
        ...
        <section className="mb-5 overflow-hidden rounded-2xl border border-white/10 bg-[#14233F]">
          <div className="flex flex-col gap-3 px-4 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div>
              <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.16em] text-emerald-400"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /> Store live · nimbus.digitalboost.shop</div>
              <h2 className="mt-2 text-xl font-semibold">Nimbus Store · lista para evolucionar</h2>
            </div>
            <button type="button" onClick={() => onNavigate?.("website-builder")} className="inline-flex h-11 items-center gap-2 rounded-lg bg-violet-500 px-4 text-sm font-medium"><Eye size={15} /> Editar en el studio</button>
          </div>
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {metrics.map((m) => (
            <button key={m.label} type="button" onClick={() => onNavigate?.(m.go)} className="rounded-xl border border-white/10 bg-[#101B32] p-4 text-left hover:border-cyan-400/40">
              <div className="flex items-center justify-between"><SignalRing value={m.pct} tone={m.tone} /><span className="inline-flex items-center gap-1 text-[11px] font-medium text-emerald-400"><ArrowUpRight size={12} /> {m.delta}</span></div>
        ...
        <div className="mt-5 grid gap-4 xl:grid-cols-[1.4fr_0.6fr]">
          <section className="rounded-xl border border-white/10 bg-[#101B32] p-4 sm:p-5">
            <div className="flex items-center justify-between"><h2 className="text-base font-semibold">Rendimiento</h2><button type="button" onClick={() => onNavigate?.("analytics")} className="text-xs text-slate-400">Analytics <ChevronRight size={12} className="inline" /></button></div>
            <div className="mt-5 flex h-36 items-end gap-1.5 rounded-lg border border-white/10 bg-[#070d18] px-3 pb-3 pt-5">{pulse.map((h, i) => (<div key={i} className="flex-1 rounded-t bg-gradient-to-t from-violet-500 to-cyan-400" style={{ height: `${h}%` }} />))}</div>
          </section>
          <section className="relative overflow-hidden rounded-xl border border-violet-500/30 bg-[#14233F] p-4 sm:p-5">
            ...
            <div className="relative">
              <div className="flex items-center gap-2 text-violet-300"><Sparkles size={14} /><span className="text-[10px] font-semibold uppercase tracking-[0.16em]">PULSE IA</span></div>
              <h2 className="mt-5 text-lg font-semibold">¿Qué quiere PULSE tocar?</h2>
              <div className="mt-5 space-y-2">
                <button type="button" onClick={() => onNavigate?.("website-builder")} className="w-full rounded-lg border border-white/10 bg-black/20 p-3 text-left"><div className="flex items-center gap-2 text-sm font-medium"><WandSparkles size={14} className="text-violet-300" /> Mejorar homepage</div></button>
                <button type="button" onClick={() => onNavigate?.("campaigns")} className="w-full rounded-lg border border-white/10 bg-black/20 p-3 text-left"><div className="flex items-center gap-2 text-sm font-medium"><Zap size={14} className="text-cyan-300" /> Crear campaña</div></button>
              </div>
            </div>
          </section>
        </div>
